...
public class Hero extends Collidable {
	 ...
	 *
	 * @param isFast true if it's a fast bubble, false for a normal bubble
	 */
	public void shootBubble(boolean isFast) {
		if (this.isAlive && !this.isPaused) {
			// spawn a bubble far enough off the hero to avoid immediate
			// collision
			int x = this.positionX + ((this.width + PRECISION) * this.direction);
			Bubble bubble = new Bubble(this.level, x, this.positionY, this.direction, isFast);
			this.level.addCollidable(bubble);
		}
	}

	/**
	 * Shoots multiple fast bubbles in rapid succession.
	 */
	public void shootMultipleBubbles(int count, boolean isFast) {
		if (this.isAlive && !this.isPaused) {
			...
			for (int i = 0; i < count; i++) {
				timer.schedule(new TimerTask() {
					@Override
					public void run() {
						int x = Hero.this.positionX + ((Hero.this.width + PRECISION) * Hero.this.direction);
						Bubble bubble = new Bubble(Hero.this.level, x, Hero.this.positionY, Hero.this.direction, isFast);
						Hero.this.level.addCollidable(bubble);
					}
				}, i * 50);
			}
		}
